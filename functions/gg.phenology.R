...
gg.phenology <- function(VC55.Co.pec) {
  
  
 # temperature.data.rows <- which(grepl("airfrost.days",VC55.weather.ss$datumAttribute))
  data.to.plot.local <- VC55.Co.pec %>% filter(YYYY >= 1960)
  data.to.plot.local$YYYYMMDD <- data.to.plot.local$YYMMDD %>% as.Date()
 ...
 # working.data$`airfrost days` <- working.data$datumValue
  ...
  working.data$count <- working.data$Quantity
...
#  temp.data.ag.by.month$YYYY <- temp.data.ag.by.month$YYYY %>% as.numeric()
  ...
  #  ggtitle("Monthly airfrost days") +
    geom_tile(aes(fill= count))  +
       scale_y_discrete(drop = FALSE) + 
    #  scale_x_discrete(drop = FALSE) + 
    scale_fill_viridis( discrete = FALSE, option = "D", trans='log10') + 
    theme_minimal() 
